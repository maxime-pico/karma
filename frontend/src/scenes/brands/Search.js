...
class Search extends React.Component {
	...
	handleChangeOrderBy(event) {
		if (event) {
			this.setState({ orderBy: JSON.parse(event.target.value) })
		}
	}
			...
			<Grid>
				...
				<Row mt={'96px'} alignItems="center">
					...
					<Col xs={12} textAlign="right" md={4}>
						{this.state.categories.length || this.state.karmas.length ? (
							<BasicButton ghost onClick={this.clearFilters}>
										...
										<form>
											{companyCategoryList.map(category => (
												<PrettyCheckbox
													...
													name="category"
													checked={
														this.state.categories.indexOf(category.id) > -1 &&
														'checked'
													}
													value={category.id}
						...
						<form>
							{Object.entries(KARMA_LABELS).map(([k, karma], index) => (
								<PrettyCheckbox
									...
									name="karma"
									checked={
										this.state.karmas.indexOf(karma.slug) > -1 && 'checked'
									}
									value={karma.slug}
					...
					<Col xs={12} md={7}>
						...
						<SearchInputForm onSubmit={this.handleSubmitSearch}>
							<SearchInput
								...
								placeholder={
									BRANDS_STATIC_CONTENTS.search_input_placeholder['fr']
								}
							/>

							{searchValue.length ? (
								<SearchInputClearButton onClick={this.clearSearchInput}>
									x
