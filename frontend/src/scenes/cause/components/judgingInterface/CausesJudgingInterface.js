				...
				{({ loading, error, data }) => {
					...
					return (
						<JudgingInterface>
							...
							>
								{(GradingMutation, { loading, error, data }) => {
									...
									return (
										<Mutation
											...
											onCompleted={() => {
												this._stopGrading(true)
											}}
										>
											{(OverallMutation, { data, error, loading }) => {
												if (loading) return 'loading'
												if (error) return 'error'
												return (
							...
							<Row mt="6px" mb="42px">
								<Col color="#A9B4CC" fontSize="12px" textAlign="center">
									{this.state.grade}
								</Col>
							</Row>
						</JudgingInterface>
