    ...
    SDLInferrer.prototype.infer = function (dbTables) {
        ...
        var types = typeCandidates.map(function (tc) {
            ...
            var fields = tc.columns.map(function (column) {
                var directives = (column.isUnique ? ["@unique"] : []).concat((column.isPrimaryKey && column.name !== 'id' ? ["@pgColumn(name: \"" + column.name + "\")"] : []), (column.defaultValue && column.defaultValue.trim() !== '[AUTO INCREMENT]'
                    ? (_this.isStringableValue(column)
                        ? ["@default(value: \"" + column.defaultValue + "\")"]
                        : ["@default(value: " + column.defaultValue + ")"]).slice() : []));
            ...
            var inlineRelationFields = inlineRelations.map(function (relation) {
                ...
                ].concat((ambiguousRelations.length > 1 && remoteColumns && remoteColumns.length > 0 ? ["@relation(name: \"" + upperCamelCase(relationName) + "\")"] : []), (selfAmbiguousRelations.length > 0 && selfRemoteColumns && selfRemoteColumns.length > 0 ? ["@relation(name: \"" + upperCamelCase(relationName) + "\")"] : []));
            ...
            var relationFields = relations.map(function (relation) {
                ...
                var selfAmbiguousRelations = ambiguousRelations.filter(function (relation) { return relation.source_table === relation.target_table; });
                var directives = (ambiguousRelations.length > 1 ? ["@relation(name: \"" + upperCamelCase(fieldName) + "\")"] : []).concat((selfAmbiguousRelations.length > 0 ? ["@relation(name: \"" + upperCamelCase(fieldName) + "\")"] : []));
            ...
            var allFields = (_.differenceWith(fields, inlineRelationFields, function (a, b) {
                return _this.removeIdSuffix(a.name) === _this.removeIdSuffix(b.name);
            })).concat(inlineRelationFields, (_.differenceWith(relationFields, relationTableFields, function (a, b) {
